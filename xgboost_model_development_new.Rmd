...
xgb_model <- xgboost(data = as.matrix(train_subset), 
                     ...
                     nrounds = 100, 
                     verbose = 0)
xgb_validation <- xgb.cv(data = as.matrix(train_subset), 
                     label = y_train, 
                     nrounds = 100, 
                     nfold = 10,
                     verbose = 0)
...
```{r}
eval_metrics <- xgb_validation$evaluation_log
plot(eval_metrics$iter, eval_metrics$train_rmse_mean, xlim = c(0,20))
points(eval_metrics$iter, eval_metrics$test_rmse_mean, col = "red")
```
